 ...
 *     ({SITE_URL}/spiritual/column/ — 単数形 + trailing slash) を出力することを担保。
 *   - 旧 /columns/ (複数形) バグが再発しないことを HTML 全体で完全排除アサーション。
 *   - page=1 / page=2 の canonical URL を pin。
 *   - 記事カードのリンクが /spiritual/column/{slug}/ を指すことを pin。
 *
 * 失敗時の合図:
 *   - canonical / 記事リンクで /columns/ (複数) が混入した
 *   - canonical が /spiritual/column/ を外れた
    ...
    articles: [
      {
        ...
        articleUrl: '/spiritual/column/article-one/',
      },
      {
        id: 'a2',
        ...
        articleUrl: '/spiritual/column/article-two/',
      },
    ],
    currentPage: 1,
    totalPages: 3,
...
describe('hub-generator URL pattern pinning (P5-44 後 regression guard)', () => {
  ...
  it('case 3: page=1 では canonical が /spiritual/column/ (trailing slash 必須)', () => {
    const html = generateHubPage(makeHubData({ currentPage: 1 }));
  ...
  it('case 4: page=2 では canonical が /spiritual/column/page/2/ 形式', () => {
    const html = generateHubPage(makeHubData({ currentPage: 2 }));
    expect(html).toContain(
  ...
  // ── case 5: 記事カードのリンクが /spiritual/column/{slug}/ を指す ───────────
  it('case 5: 記事カードの href が /spiritual/column/{slug}/ (相対 or 絶対) を指す', () => {
    const html = generateHubPage(makeHubData({ currentPage: 1 }));
    ...
    for (const href of cardHrefs) {
      // 相対 or 絶対のいずれでも /spiritual/column/{slug}/ パターンに一致
      expect(href).toMatch(
        /^(https?:\/\/[^/]+)?\/spiritual\/column\/[a-z0-9-]+\/$/,
      );
      // 旧バグの再発防止
  ...
  it('case 6: ナビゲーション「コラム一覧」リンクが /spiritual/column/ を指す', () => {
    const html = generateHubPage(makeHubData({ currentPage: 1 }));
