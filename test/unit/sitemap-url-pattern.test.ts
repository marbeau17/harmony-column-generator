...
describe('sitemap.ts 記事 URL パターン pin (P5-44)', () => {
  beforeEach(() => {
    // env をデフォルトに固定 (default = https://harmony-mc.com + /spiritual/column)
  ...
  it('case 1: 記事 entry が新形式 /spiritual/column/{slug}/ を含む (canonical と同じ trailing slash)', async () => {
  ...
  it('case 2: 旧形式 /column/{slug} (ホスト直後・/spiritual 抜け) が一切混入しない', async () => {
    const { default: sitemap } = await import('@/app/sitemap');
    const entries = await sitemap();

    for (const f of mocks.fixture) {
      const oldPattern1 = `https://harmony-mc.com/column/${f.slug}`;
      const oldPattern2 = `https://harmony-mc.com/column/${f.slug}/`;
      for (const entry of entries) {
        expect(entry.url).not.toBe(oldPattern1);
        expect(entry.url).not.toBe(oldPattern2);
      }
    }
  });
  ...
  it('case 3: getSiteUrl() + getHubPath() の組み合わせで URL が組まれている', async () => {
    ...
    expect(articleEntry!.url).not.toMatch(
      /^https:\/\/harmony-mc\.com\/column\//,
    );
  });

  it('case 4: 記事 URL すべてに `/spiritual/column/` substring が含まれる', async () => {
    const { default: sitemap } = await import('@/app/sitemap');
    const entries = await sitemap();
    ...
    for (const url of articleUrls) {
      expect(url).toContain('/spiritual/column/');
