...
## step9 完了サマリ — Publish Control V2 legacy 削除 (P5-43 後始末)

**Date:** 2026-05-17
**Author:** Generator/Fixer
**Spec:** docs/specs/publish-control/SPEC.md §4 step9

### 1. 背景
P5-43 step8 (RLS 切替マイグレ) を 2026-04-25 にデプロイし、22 日経過 (SPEC 規定の 14〜30 日窓内、期限 2026-05-09 を 8 日超過)。可視判定が完全に `visibility_state` に統一済みであることを 22 日間の本番稼働で確認したため、`PUBLISH_CONTROL_V2` フラグおよび関連 legacy コードを撤去。

### 2. 変更内容
- **UI legacy 撤去**: `src/app/(dashboard)/dashboard/articles/page.tsx` の `PUBLISH_CONTROL_V2 ? PublishButton : <checkbox>` 三項を解体し、PublishButton 単独に統一 (~100 行削減)
- **API gate 撤去**: `visibility` / `review` / `batch-hide-source` 3 route から `isPublishControlEnabled()` チェック + 404 早期 return を削除
- **feature-flag module 削除**: `src/lib/publish-control/feature-flag.ts` を撤去 (production caller 0)
- **dead code 撤去**:
  - `ArticlesResponse.count` `@deprecated` field + json.count fallback
  - `CtaConfig.bannerUrl` / `bannerAlt` (consumer 0、validators / seed も同時整理)
  - `src/lib/publish-control/runtime-parity.ts` + test (P5-43 step 2 シャドー期間専用、prod caller 0)
- **テスト整理**:
  - 3 unit test の `vi.mock('@/lib/publish-control/feature-flag')` 削除
  - `batch-hide-source-api.test.ts` の "Case 2: PUBLISH_CONTROL_V2 未設定 → 404" を削除 (gate 自体が消失)
  - 全 unit test の `process.env.PUBLISH_CONTROL_V2 = 'on'` 設定を削除
  - `test/e2e/hub-rebuild.spec.ts` 削除 (LEGACY checkbox UI 専用テスト、§6.3 ハブ再生成は monkey-publish-control S2 + PublishButton で代替済み)
  - `test/e2e/helpers/monkey-fixtures.ts` の `PUBLISH_CONTROL_V2!=on` guard 削除
  - `batch-hide-source.spec.ts` の REQUIRED_ENV から `PUBLISH_CONTROL_V2` 除外
  - `publish-control-baseline.spec.ts` の B5 を `[401, 404]` 受容 → `401` 単独に厳格化
- **.env.local.example 整理**: `PUBLISH_CONTROL_V2` / `NEXT_PUBLIC_PUBLISH_CONTROL_V2` エントリ削除

### 修正したファイル
- (deleted) `src/lib/publish-control/feature-flag.ts`
- (deleted) `src/lib/publish-control/runtime-parity.ts`
- (deleted) `test/unit/runtime-parity.test.ts`
- (deleted) `test/e2e/hub-rebuild.spec.ts`
- (modified) `src/app/(dashboard)/dashboard/articles/page.tsx`
- (modified) `src/app/api/articles/[id]/visibility/route.ts`
- (modified) `src/app/api/articles/[id]/review/route.ts`
- (modified) `src/app/api/articles/batch-hide-source/route.ts`
- (modified) `src/lib/content/cta-generator.ts`
- (modified) `src/lib/validators/settings.ts`
- (modified) `scripts/seed.ts`
- (modified) `.env.local.example`
- (modified) 3 unit test + 4 E2E spec + 1 helper

### 検証結果
- `tsc --noEmit` exit 0
- `npm run build` PASS (全 routes コンパイル成功)
- 影響テスト 32/32 PASS (visibility-route-states 10 / visibility-hallucination-gate 3 / review-api 12 / batch-hide-source-api 7)
- 既存 failing 2 件 (`stage1-zero-outline` / `zero-gen-logging`) は pre-existing で本変更と無関係 (main HEAD でも fail を確認済み)

### 関連雑ノート
- 削除した `hub-rebuild.spec.ts` の §6.3 シナリオ (uncheck → hub から消える / bulk 0 件で hub 再生成 / 失敗時 banner 表示) は: §6.3.1 = monkey-publish-control S2、§6.3.2/§6.3.3 = PublishButton 内 toast.error が代替
- shadow 期間用 `assertStateParity()` は P5-43 step 4 で audit-only 降格された時点で実質死コード化していた

### 次のステップ
1. SPEC §4 step9 を完了 ✅ にマーク済み
2. publish-control SPEC 全体を読み返し、step6〜8 関連の transient guidance も折り畳む余地を検証 (次サイクル)

---

## P5-105 / P5-106 / P5-107 完了サマリ — agent ラベル動的紐付け + normalizer 経路統一 + スキーマ統合計画

**Date:** 2026-05-17
**Author:** Generator/Fixer (5-agent ループ短縮版 — 軽量タスク 3 件)

### 1. 背景
P5-103/P5-104 の next-cycle 候補 5 件のうち、ハンズオン 4 件を 1 セッションでクローズ。step9 (#17) は別エントリ。
- **P5-105 = #18**: P5-103 で導入した `QUEUE_AGENT_LABELS` は静的固定値だったため、env で切替えた AI モデル名 (例: `gemini-3.1-pro-preview` → `gemini-4-pro-experimental`) が UI に反映されない
- **P5-106 = #19**: P5-104 の normalizer 経路が `queue/process` images ステップ "だけ" に適用されていた。`/api/articles/[id]/generate-images` と `/api/articles/batch-generate-images` で dual-schema fallback と silent skip + 'hero' 自動補完が温存されており、P5-104 が物理的に封じ込めようとした無音失敗が裏口から漏れる構造
- **P5-107 = #20**: 画像 prompt スキーマは実は 3 系統並存 (`position` / `section_id` / `slot`) で normalizer は 2 系統しか対応していなかった。中期的な廃止計画を策定

### 2. 変更内容

#### P5-105 (agent ラベル動的紐付け)
- (added) `src/lib/queue/agent-labels.ts` — `composeAgentLabel(role, step)` で role × step kind (text/image/none) × env (`GEMINI_MODEL` / `GEMINI_IMAGE_MODEL`) を結合して `"AI ライター (gemini-3.1-pro-preview)"` 形式を生成
- (modified) `src/app/api/queue/route.ts` — list レスポンスに `current_agent_display` を追加 (server-side で composer 呼び出し、env をクライアントに漏らさない)
- (modified) `src/app/(dashboard)/dashboard/planner/page.tsx` — `current_agent_display` を優先表示、static map (`QUEUE_AGENT_LABELS`) はフォールバック
- (added) `test/unit/agent-labels.test.ts` — 11 ケース全 PASS (env 上書き反映、未知 role フォールバック、AI 非関与 (Publisher) で model 名抑止、step=null 動作 など)

#### P5-106 (normalizer 経路統一)
- (modified) `src/app/api/articles/[id]/generate-images/route.ts` — inline dual-schema fallback (`raw.position || raw.section_id || 'hero'`) と silent skip (`if (!prompt) continue`) を撤去、`normalizeImagePrompts()` 経由に統一。normalize 失敗時は 400 で errorMessage 明示
- (modified) `src/app/api/articles/batch-generate-images/route.ts` — 配列キャスト (`as { prompt, position, alt_text_ja? }[]`) を撤去、`normalizeImagePrompts()` 経由に統一。記事単位の normalize 失敗は errors 配列に格納して進行継続
- (modified) `src/lib/content/image-prompts-normalizer.ts` — 第 3 スキーマ `slot` を accept、優先順位 `position > section_id > slot`、エラーメッセージを `"position/section_id/slot が未指定"` に更新
- (modified) `test/unit/image-prompts-normalizer.test.ts` — `slot` 形式 4 ケース追加 (slot=hero/body/summary, position 優先, section_id 優先)、計 14 → 18 ケース全 PASS

#### P5-107 (スキーマ統合計画)
- (added) `docs/refactor/image-prompts-schema-unification.md` — 3 フェーズ計画 (Phase 1 観察 30 日 → Phase 2 AI prompt 改修 30 日 → Phase 3 normalizer から legacy field 撤去)
- (modified) `src/lib/content/image-prompts-normalizer.ts` 冒頭コメント — 「2 系統」→「3 系統」記述更新

### 修正したファイル
- (added) `src/lib/queue/agent-labels.ts`
- (added) `test/unit/agent-labels.test.ts`
- (added) `docs/refactor/image-prompts-schema-unification.md`
- (modified) `src/app/api/queue/route.ts`
- (modified) `src/app/(dashboard)/dashboard/planner/page.tsx`
- (modified) `src/app/api/articles/[id]/generate-images/route.ts`
- (modified) `src/app/api/articles/batch-generate-images/route.ts`
- (modified) `src/lib/content/image-prompts-normalizer.ts`
- (modified) `test/unit/image-prompts-normalizer.test.ts`

### 検証結果
- `tsc --noEmit` exit 0
- `npm run build` PASS (全 routes コンパイル成功)
- vitest: **1000/1002 PASS** (87/89 files)。失敗 2 件 (`stage1-zero-outline.test.ts` / `zero-gen-logging.test.ts`) は本セッション前から存在する pre-existing failure (main HEAD でも fail 確認済み、本変更と無関係)
- 新規追加 15 ケース (agent-labels 11 + normalizer slot 4) 全 PASS
- step9 影響テスト 32/32 PASS (visibility-route-states 10 / visibility-hallucination-gate 3 / review-api 12 / batch-hide-source-api 7)

### 関連雑ノート
- P5-106 で発見した「P5-104 の防御層が `queue/process` だけに局所化されていた」事象は、`feedback_systemic_antipatterns.md` #3 (同一ロジック複数ファイル禁止) の典型例。今回 3 経路すべて normalizer 1 本に統合
- P5-107 の Phase 3 (normalizer から legacy field 撤去) は前提条件「30 日間 legacy schema 発火 0」を満たしてからの実施。Phase 1 観察ログを次サイクル Generator が追加

### 次のステップ
1. Phase 1 観察ログ (`logger.info('ai', 'image_prompt.legacy_schema', { kind })`) を normalizer に追加 (次サイクル、rate-limit 必須)
2. 30 日後 (2026-06-15) に Vercel ログ集計 → Phase 2 (AI prompt 改修) 可否を判定
3. 本サイクルで完了した 4 件 (#17〜#20) のクローズドループ証跡を `eval_report.md` に追記
