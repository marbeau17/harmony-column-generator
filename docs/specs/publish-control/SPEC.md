...
| Step | 内容 | 状態 |
|:---|:---|:---|
| **Step 1** | スキーマ拡張・state-machine に `draft` / `pending_review` 追加（additive） | **本 Step（完了）** |
| Step 2 | reader 8 箇所を `visibility_state` ベースに置換、parity 検証スクリプト導入 | 未着手 |
| Step 3 | writer 統一・`pending_review` を zero-gen 完了フローに導入 | 未着手 |
| Step 4 | `reviewed_at` を audit-only に降格（状態判断から完全切り離し） | 未着手 |
