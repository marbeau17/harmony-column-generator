...
<!-- P5-43 Step 1: state-machine 拡張 -->
### 3.7 ステートマシン現状（P5-43 Step 1 拡張後）

> 設計詳細・全 state 表・推奨案根拠は `docs/refactor/publish-control-unification.md` §2.2 / §2.3 を参照。
> 本セクションは additive な現状スナップショット。後続 Step 2〜5 で reader/writer/列削除を順次反映する。

**実装位置:** `src/lib/publish-control/state-machine.ts`

#### TRANSITIONS マップ（実装値・2026-05-02 時点）

| from | 許可遷移先 | 補足 |
|:---|:---|:---|
| `draft` | `pending_review` | submit_for_review |
| `pending_review` | `idle`, `draft` | approve_review / reject_review |
| `idle` | `deploying`, `pending_review` | deploy / 再レビュー戻し |
| `deploying` | `live`, `live_hub_stale`, `failed` | デプロイ結果 |
| `live` | `deploying` | 再デプロイ・unpublish 経由 |
| `live_hub_stale` | `deploying` | hub rebuild 再試行 |
| `unpublished` | `deploying` | 再公開 |
| `failed` | `deploying`, `idle` | リトライ or 手動回復 |
...
| `idle` | × | 承認済み・公開可能、未デプロイ |
| `deploying` | × | デプロイ進行中（60s で dangling 判定） |
| `live` | ◯ | 公開中・ハブ整合 |
| `live_hub_stale` | ◯ | 記事 live・ハブ rebuild 失敗 |
| `unpublished` | × | ソフト撤回済み（noindex notice） |
| `failed` | × | デプロイ失敗・回復待ち |

> 詳細表（sitemap・FTP 物理ファイルの有無等）は
> [`docs/refactor/publish-control-unification.md` §2.3](../../refactor/publish-control-unification.md#23-各状態の定義) を参照。

#### `reviewed_at` 段階的降格計画

`reviewed_at` は **Step 1 完了後も書き込み・読み取りとも従来通り稼働中**。
推奨案 b（pending_review 中間ステート + audit 保持）に従い、以下 Step で段階的に降格する:

| Step | 内容 | 状態 |
|:---|:---|:---|
| **Step 1** | スキーマ拡張・state-machine に `draft` / `pending_review` 追加（additive） | 完了 |
| Step 2 | reader 8 箇所を `visibility_state` ベースに置換、parity 検証スクリプト導入 | 完了 |
| Step 3 | writer 統一・`pending_review` を zero-gen 完了フローに導入 | 完了 |
| **Step 4** | `reviewed_at` を audit-only に降格（状態判断から完全切り離し） | **完了 (reviewed_at audit-only)** |
| Step 5 | 旧 `reviewed_by` 列の削除検討・lint/CI ガード強化 | 未着手 |

> Step 4 完了 (reviewed_at audit-only): 2026-05-02。
> writer 経路は Step 3 で `POST /api/articles/[id]/review` (action='approve') に集約済み。
> Step 4 では display 用 reader (一覧 title 属性 / 詳細ページ確認済みバッジ / deploy ログ) と
> 型定義 (`src/types/article.ts`, `src/lib/db/articles.ts`,
> `src/lib/articles/fetch-published-articles.ts`,
> `src/app/(dashboard)/dashboard/articles/page.tsx`) に `// audit-only` コメントを付与し、
> 状態判定からの完全切り離しを明示。`compute-centroid.ts` は verify-state-parity 検証用に
> select 句に残置 (where では不使用)。詳細は
> [`docs/refactor/publish-control-unification.md` §5 Step 4](../../refactor/publish-control-unification.md#step-4--reviewed_at-を-audit-only-に降格) を参照。

各 Step の詳細は [`docs/refactor/publish-control-unification.md` §5 マイグレーション手順](../../refactor/publish-control-unification.md#5-マイグレーション手順) を参照。
<!-- /P5-43 Step 1: state-machine 拡張 -->

<!-- P5-43 Step 3: review action API (additive) -->
### 3.8 review action API (P5-43 Step 3 で導入)

> 設計詳細は `docs/refactor/publish-control-unification.md` §4.1 / §5 Step 3、
> 完了確認は `docs/refactor/step3-completion-checklist.md` を参照。
> 本セクションは additive な仕様追加であり、既存 §3.3 の `:id/visibility` API・
> `:bulk` API・冪等性ルールは変更されない。

#### 目的
Step 3 までは `reviewed_at` の書込が 5 箇所 (visibility API 副作用 / zero-gen 完了 /
batch-hide / dashboard PUT / ad-hoc スクリプト) に分散していた。これを
**新 API 1 本** に集約し、`reviewed_at` を「人間レビューの事実時刻」を示す
audit-only 列へ降格 (Step 4 で完成) するための足場を作る。
...
200 OK            → 状態遷移成功 (publish_events に INSERT 済み)
422 Unprocessable → 期待 state と異なる ({ code: 'INVALID_TRANSITION' }) /
                    requestId 欠落 ({ code: 'MISSING_REQUEST_ID' })
423 Locked        → 同記事を他リクエストが処理中 (PG advisory lock 競合)
401 Unauthorized  → 未認証
403 Forbidden     → 該当記事に対する書込権限なし (RLS)
```

#### action 別の状態遷移と副作用

| action | 期待 from | to | reviewed_at | reviewed_by | publish_events.action |
|:---|:---|:---|:---:|:---:|:---|
| `submit` | `draft` | `pending_review` | 不変 | 不変 | `review_submit` |
| `approve` | `pending_review` | `idle` | `now()` | actor_id | `review_approve` |
| `reject` | `pending_review` | `draft` | 不変 | 不変 | `review_reject` |

- `state-machine.ts` の `assertTransition()` を経由して遷移を強制
- `approve` パスが **`reviewed_at` / `reviewed_by` を書く唯一の経路**
  (Step 3 完了以降は visibility API・zero-gen・batch-hide からの書込は禁止)
- `reject` / `submit` では `reviewed_at` を変更しない (audit 履歴を保持)

#### 冪等性 / 原子性
- §3.3 と同方式の `requestId` (ULID) で重複短絡
- 記事ごとの PG advisory lock (`pg_try_advisory_xact_lock(article_id::bigint)`)
- 競合時は 423 を返却

#### HTML History Rule との関係
本 API は HTML を書き換えないため `article_revisions` への INSERT は不要。
監査ログは `publish_events` のみで完結する (HTML History Rule の対象外)。

#### `publish_events.action` CHECK 制約拡張
Step 3 マイグレーション `20260502_publish_control_step3.sql` で以下 3 値を additive 追加:
- `'review_submit'`
- `'review_approve'`
- `'review_reject'`
...
#### zero-gen 完了フローでの利用
- `autoApprove=true` 経路: 内部で `visibility_state='idle'` を直接セットし、
  `publish_events` に `action='review_approve'` (actor=system) を INSERT
- `autoApprove=false` 経路: `visibility_state='pending_review'` をセットし、
  `action='review_submit'` (actor=system) を INSERT

#### FTP deploy ゲートとの統合
- `pending_review` 状態の記事に対する `:id/deploy` は引き続き 422 を返す
  ({ code: 'PENDING_REVIEW' })
- `idle` / `failed` / `live_hub_stale` / `unpublished` のみ deploy 許可
  (`docs/refactor/publish-control-unification.md` §6.1 リスク表参照)
