...
## 第 6 サイクル — P5 Zero-Generation V1（基盤 + 統合 + UI + 検証層）

**Date:** 2026-04-30
**Author:** Evaluator H19 (subagent role)

### サマリ
| 範囲 | 件数 | 結果 |
|---|---|---|
| 単体テスト | 31 ファイル / 393 件 | **PASS**（全件、Duration 2.58s） |
| 型チェック | exit=0 | **PASS** |
| ビルド | 9 新ルート全出現 + Compiled successfully | **PASS** |
| AC-P5-1〜17 | 17 件 | **全 PASS** |
| AC-P5-18〜22 | 5 件 | **PENDING**（実機検証待ち） |

### 実行ログ要点
- `npx vitest run`: `Test Files 31 passed (31) / Tests 393 passed (393)` — 前 87 件 + P5 で +306 件追加分すべて緑。
- `npx tsc --noEmit -p tsconfig.json`: `exit=0`（型エラー 0）
- `npm run build`: `Compiled successfully` 後、ルート一覧に下記 9 件全て出現確認:
  - ƒ /api/articles/[id]/hallucination-check
  - ƒ /api/articles/[id]/regenerate-segment
  - ƒ /api/articles/zero-generate
  - ƒ /api/articles/zero-generate-full
  - ƒ /api/articles/zero-generate/[job_id]/progress
  - ƒ /api/hallucination-retry
  - ƒ /api/publish-events
  - ƒ /dashboard/articles/new-choice
  - ƒ /dashboard/articles/new-from-scratch
- 既知の `Dynamic server usage` ログ（/api/queue, /api/settings, /api/source-articles）は従前から発生している警告で、ビルド成果物は正常生成（第 5 サイクルでも同条件）。本サイクル変更とは無関係。

### AC-P5 達成度マトリクス（spec §15）
| AC | 内容 | 判定 | 根拠 |
|---|---|---|---|
| AC-P5-1 | zero-generate API 動作 | **PASS** | unit test 通過、ビルドにルート出現 |
| AC-P5-2 | generation_mode='zero' で保存 | **PASS** | migration + integration test |
| AC-P5-3 | hallucination/yukiko_tone/readability 各 score 記録 | **PASS** | スキーマ + テスト |
| AC-P5-4 | article_claims に 4 タイプ記録 | **PASS** | テスト緑 |
| AC-P5-5 | critical claim ≥1 で PublishButton disabled | **PASS** | visibility gate テスト |
| AC-P5-6 | stage2-writing.ts 再利用 + few-shot 動的選定 | **PASS** | 該当テスト緑 |
| AC-P5-7 | yukiko_tone_score ≥0.80 で公開可 | **PASS** | tone scoring テスト |
| AC-P5-8 | ""禁止 / スピ断定回避 必須通過 | **PASS** | validator テスト |
| AC-P5-9 | クリシェ 5 件以上で block | **PASS** | テスト緑 |
| AC-P5-10 | lead_summary 100-150 字 + citation 3 件 | **PASS** | outline schema テスト |
| AC-P5-11 | タイトル 28-35 字 / meta 120 字 | **PASS** | outline + validator テスト |
| AC-P5-12 | Article+FAQ+Person+BreadcrumbList JSON-LD | **PARTIAL** | 既存実装あり、ゼロ生成記事への適用は次サイクル |
| AC-P5-13 | new-from-scratch ページ存在 | **PASS** | ビルドにルート出現 |
| AC-P5-14 | プログレス Stepper 表示 | **PASS** | UI テスト緑 |
| AC-P5-15 | ハルシネ結果ペインで claim ハイライト | **PASS** | UI テスト緑 |
| AC-P5-16 | 単体テスト全件 PASS | **PASS** | 393/393 |
| AC-P5-17 | tsc / build PASS | **PASS** | exit=0 + Compiled successfully |
| AC-P5-18 | E2E ZG-1〜4 PASS | **PENDING** | H16 結果次第 |
| AC-P5-19 | 既存 E2E 回帰なし | **PENDING** | H16 結果次第 |
| AC-P5-20 | 1 記事 P95 < 90s | **PENDING** | 実機計測未 |
| AC-P5-21 | バッチ 10 記事 < 5 分 | **PENDING** | 実機計測未 |
| AC-P5-22 | 月次コスト < $15 | **PENDING** | 実 embedding 未投入 |

### 専門家観点 5 軸（spec §15）
- 機能完全性: **4/5**（AC-P5-12 PARTIAL + AC-P5-18〜22 PENDING）
- 動作安定性: **5/5**（単体 393/393, 型 0, ビルド成功）
- 仕様の妥当性: **5/5**（spec §15 の 17/22 が静的検証で確定 PASS、残 5 は実機事項）
- 回帰なし: **5/5**（既存 publish-control / dangling-recovery / hub-rebuild 系のテスト全件緑、デグレ 0）
- 統合性: **5/5**（9 新ルート + 2 ダッシュボードページ全て build artifact に着地）

### 総合判定
**【クローズドループ完了 — P5-1〜P5-7 PASS（実機検証 AC-P5-18〜22 のみ PENDING）】**

CLAUDE.md 評価軸 4-5/5 充足。AC-P5-12 は既存 JSON-LD 基盤がありゼロ生成記事適用のみ次サイクル送り、AC-P5-18〜22 は実機検証必須項目で本サイクル静的範囲外のため PENDING でも本サイクル合格判定は妨げない。

### 重要な発見
- F2/F4/F5 の DI 設計により並列開発が機能（後続 Fixer の遅延着地に対応）。F4 の DI 注入式 judge により F2（RAG）未完でも単体動作可能だった。
- H1 G1 で発覚した `match_source_chunks` RPC 不足を H1 で追加マイグレで解消済（migrations 系も第 5 サイクル後の 20260419 系まで含めて整合）。
- ビルド時 `unhandledRejection PageNotFoundError: /_document` が表示されるが、Compiled successfully 後の段階で App Router 構成においては既知無害（Pages Router 互換探索の副作用）。成果物（Route 一覧）は完全。
- H16 で実機検証時、既存 E2E 回帰確認（AC-P5-19）が必須。publish-control monkey と hub-rebuild spec への影響を重点監視。

### 次サイクル候補
1. **AC-P5-18〜22 確定**（H16 実機 E2E + GEMINI_API_KEY 投入 + P95/バッチ計測）
2. **本番マイグレ適用**（20260501 / 20260502 系の P5 用テーブル）
3. **1499 記事 embedding 投入**（H11 強化版で）
4. **Vercel/GitHub 環境変数追加**（HALLUCINATION_RETRY_TOKEN 等）
5. **本番デプロイ + スモーク**（P5-8）
