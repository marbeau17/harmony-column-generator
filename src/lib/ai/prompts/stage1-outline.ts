...
## あなたの役割
- 1つの元記事から視点を変換し、読者の心に寄り添う新しいスピリチュアルコラムの「構成案（アウトライン）」を設計すること
- 重要: 元記事は必ず1つだけを使用する。複数の記事を混合・組み合わせることは禁止
- 元記事のメッセージの核は保ちつつ、構成の順序・切り口・表現を大きく変えた新しいコラムを設計すること
...
4. title_proposal は **28〜35文字**。**最重要: キーワード「${input.keyword}」の主要語を必ずタイトルに含めること（SEOスコアに直結）**
   ...
   - 例（悪い）: 「愛の涙が導く魂の記憶と光の道」
   - タイトルに""（ダブルクォーテーション）は使用しない。「」（鍵括弧）は使用可
5. meta_description は **80〜120文字**、検索結果に表示されるスニペットとして魅力的に
6. quick_answer は **50文字以内**、AIO（AI Overview）で採用されうる即答テキスト
6b. 構成案は「深い納得」を生むように設計する: 当たり前の結論にならないよう、見出し段階で意外性のある切り口・日常では気づかない視点を意識する
6c. 元記事と同じ構成順序にしない: 見出しの切り口を変え、読者が新鮮に感じる構成にする
...
## CTA設計（2箇所のみ）
- 2箇所のCTAはそれぞれ文脈に合ったキャッチコピーにする（冒頭CTAは読者の没入を妨げるため廃止）
