...
// P5-65: keyword 修復は「実際に keyword を本文に挿入し、修正済 stage2_body_html 全文を返す」
// ことを強く明示。トークン分割（"気功 自然 東洋医学" → ["気功","自然","東洋医学"]）を
// オーケストレータ側で行い、各トークン毎に最低 3 回挿入を要求する。
export function buildKeywordFixPrompt(args: {
  ...
  const system = [
    COMMON_SYSTEM,
    '指示: 指定された各キーワード（トークン）を、本文中に最低 3 回ずつ、自然な日本語の文脈で確実に挿入する。',
    '出力: 修正済みの **stage2_body_html 全文** を {"html": "..."} で返す。一部抜粋・差分・省略は禁止。',
    '禁止: 強引な挿入、不自然な羅列、見出しへのキーワード乱挿入、本文の大幅短縮、構造の改変。',
    '推奨: 既存段落の流れを尊重し、関連する具体例や説明の中で自然に言及する。',
  ].join('\n');
  const user = [
    `# 挿入すべきキーワード（各トークンごとに本文中で最低 3 回）`,
    args.keywords.map((k, i) => `${i + 1}. ${k}`).join('\n'),
    ``,
    `# 要件（厳守）`,
    `- 上記の各キーワードを、本文中に最低 3 回ずつ自然な文脈で登場させる（例: "気功"・"自然"・"東洋医学" がそれぞれ 3 回以上）`,
    `- 出力は修正済みの stage2_body_html 全文（H2/H3/段落/画像/CTA を完全保持）`,
    `- 既存 H2/H3 構造は維持し、新規見出しを追加しない`,
    `- 「キーワード」「SEO」のようなメタ言及は禁止`,
    `- 元の本文より極端に短くしない（同等以上の文量を保つ）`,
    ``,
    `# 本文HTML`,
    args.bodyHtml,
  ].join('\n');
